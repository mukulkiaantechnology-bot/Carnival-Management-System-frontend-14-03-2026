...
const Invoice = () => {
    ...
    const getStatusClass = (status) => {
        switch (status) {
            case 'Partially Paid': return 'Invoice-partial';
        ...
        return (
            <div className="Invoice-invoice-full-page-view">
                ...
                <div className="Invoice-view-content-wrapper Invoice-printable-area">
                    <div
                        className={`invoice-preview-container template-${(companyDetails.template || 'Invoice-newyork').toLowerCase().replace(/\s+/g, '')}`}
                        id="invoice-print-content"
                        style={{ '--header-bg': companyDetails.color || '#004aad' }}
                    >
                        <div className="Invoice-invoice-header-wrapper">
                            <div className="Invoice-invoice-preview-header">
                                <div className="Invoice-invoice-header-left">
                                    {companyDetails.logo ? (
                                        <img src={companyDetails.logo} alt="Company Logo" className="Invoice-invoice-logo-large" />
                                    ) : (
                                        <h2 style={{ color: companyDetails.color, margin: 0, textTransform: 'uppercase' }}>{companyDetails.name}</h2>
                                    )}

                                    <div className="Invoice-invoice-company-details">
                                ...
                                <div className="Invoice-invoice-header-right">
                                    <div className="Invoice-invoice-title-large">INVOICE</div>
                                    <div className="Invoice-invoice-meta-info">
                                        <div className="Invoice-invoice-meta-row">
                                            <span className="Invoice-invoice-label">Number:</span> #{selectedInvoice?.invoiceNumber || 'N/A'}
                                        </div>
                                        <div className="Invoice-invoice-meta-row">
                                            <span className="Invoice-invoice-label">Issue:</span> {selectedInvoice?.date ? new Date(selectedInvoice.date).toLocaleDateString() : 'N/A'}
                                        </div>
                                        <div className="Invoice-invoice-meta-row">
                                            <span className="Invoice-invoice-label">Due Date:</span> {selectedInvoice?.dueDate ? new Date(selectedInvoice.dueDate).toLocaleDateString() : 'N/A'}
                                        </div>
                                    </div>
                                    {companyDetails.showQr && (
                                        <div className="Invoice-invoice-qr-box">
                                            <img src={`https://api.qrserver.com/v1/create-qr-code/?size=100x100&data=${selectedInvoice?.invoiceNumber || 'Invoice'}`} alt="QR" className="Invoice-invoice-qr-code" />
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="Invoice-invoice-addresses">
                            <div className="Invoice-invoice-bill-to">
                                <div className="Invoice-invoice-section-header">Bill To:</div>
                            ...
                            <div className="Invoice-invoice-ship-to" style={{ textAlign: 'right' }}>
                                <div className="Invoice-invoice-section-header">Ship To:</div>
                        ...
                        <table className="Invoice-invoice-table-preview">
                            <thead>
                                <tr>
                                    <th>Item</th>
                                    <th>Warehouse</th>
                                ...
                                {((selectedInvoice?.invoiceitem || selectedInvoice?.items) || []).map((item, idx) => (
                                    <tr key={idx}>
                                        <td>
                                            ...
                                            <div className="Invoice-text-xs Invoice-text-slate-500">{item.description}</div>
                                        </td>
                        ...
                        <div className="Invoice-invoice-total-section">
                            <div className="Invoice-invoice-totals">
                                <div className="Invoice-invoice-total-row">
                                    <span>Sub Total</span>
                                ...
                                <div className="Invoice-invoice-final-total">
                                    <span>Total</span>
                        ...
                        {companyDetails.notes && (
                            <div style={{ marginTop: '2rem', borderTop: '1px solid #e2e8f0', paddingTop: '1rem' }}>
                                <h3 className="Invoice-invoice-section-header">Notes</h3>
                ...
                <div className="Invoice-table-container">
                    <table className="Invoice-invoice-table">
                        ...
                        <tbody>
                            {invoices.map(inv => (
                                <tr key={inv.id}>
                                    <td className="font-bold text-blue-600 Invoice-cursor-pointer" onClick={() => handleView(inv)}>{inv.invoiceNumber}</td>
                                    <td>{inv.customer?.name}</td>
                                    <td>{new Date(inv.date).toLocaleDateString()}</td>
                                    <td>{new Date(inv.dueDate).toLocaleDateString()}</td>
                                    <td className="font-bold">{formatCurrency(inv.balanceAmount)}</td>
                                    <td><span className={`Invoice-invoice-status-pill ${getStatusClass(inv.status || 'Pending')}`}>{inv.status || 'Pending'}</span></td>
                                    <td className="text-right">
                                        <div className="Invoice-invoice-action-buttons">
                                            <button className="Invoice-invoice-action-btn Invoice-view" onClick={() => handleView(inv)}><Eye size={16} /></button>
                                            <button className="Invoice-invoice-action-btn Invoice-edit" onClick={() => handleEdit(inv)}><Pencil size={16} /></button>
                                            <button className="Invoice-invoice-action-btn Invoice-delete" onClick={() => handleDelete(inv)}><Trash2 size={16} /></button>
                                        </div>
                                    </td>
                                </tr>
                            ))}
                ...
                <div className="Invoice-modal-overlay">
                    <div className="Invoice-modal-content Invoice-invoice-form-modal">
                        ...
                        <div className="Invoice-modal-body-scrollable">
                            ...
                            {/* Top Section */}
                            <div className="Invoice-form-section-grid">
                                <div className="Invoice-company-section">
                                    <div className="Invoice-logo-upload-box">
                                        {companyDetails.logo ? (
                                            <img src={companyDetails.logo} alt="Company Logo" style={{ maxWidth: '100%', maxHeight: '50px', objectFit: 'contain' }} />
                                        ) : (
                                            <h1 className="Invoice-company-logo-text">BOOK</h1>
                                        )}
                                    </div>
                                    <div className="Invoice-company-inputs">
                                        <input type="text" className="Invoice-full-width-input Invoice-user-editable"
                                ...
                                <div className="Invoice-meta-section">
                                    ...
                                    <div className="Invoice-status-indicator" style={{ color: '#ef4444', borderColor: '#ef4444' }}>
                                        UNPAID
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="Invoice-customer-section">
                                <div className="Invoice-form-group mb-2">
                                    <label className="Invoice-form-label-sm">Bill To</label>
                                    <select className="Invoice-form-select-large" value={customerId} onChange={(e) => {
                                ...
                                <div className="Invoice-customer-details-grid">
                                    <input type="text" placeholder="Billing Address" className="Invoice-detail-input"
                                        value={customerDetails.address} onChange={(e) => setCustomerDetails({ ...customerDetails, address: e.target.value })} />
                                    <input type="email" placeholder="Email Address" className="Invoice-detail-input"
                                        value={customerDetails.email} onChange={(e) => setCustomerDetails({ ...customerDetails, email: e.target.value })} />
                                    <input type="tel" placeholder="Phone Number" className="Invoice-detail-input"
                                        value={customerDetails.phone} onChange={(e) => setCustomerDetails({ ...customerDetails, phone: e.target.value })} />
                                </div>
                            </div>
                            {/* Items Table */}
                            <div className="Invoice-items-section-new">
                                <button className="Invoice-btn-add-row" onClick={addItem}>
                                    <Plus size={14} /> Add Line Item
                                </button>
                                <div className="Invoice-table-responsive">
                                    <table className="Invoice-new-items-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            {items.map(item => (
                                                <tr key={item.id}>
                                                    <td>
                                                        <select className="Invoice-full-width-input"
                                                    ...
                                                    <td>
                                                        <select className="Invoice-full-width-input" value={item.warehouseId} onChange={(e) => updateItem(item.id, 'warehouseId', e.target.value)}>
                                                            <option value="">Select Warehouse...</option>
                                                            {allWarehouses.map(w => <option key={w.id} value={w.id}>{w.name}</option>)}
                                                        </select>
                                                    </td>
                                                    <td>
                                                        <input type="number" className="Invoice-qty-input" value={item.qty}
                                                            onChange={(e) => updateItem(item.id, 'qty', e.target.value)} />
                                                    </td>
                                                    <td>
                                                        <input type="number" className="Invoice-rate-input" value={item.rate}
                                                            onChange={(e) => updateItem(item.id, 'rate', e.target.value)} />
                                                    </td>
                                                    <td>
                                                        <input type="number" className="Invoice-tax-input" value={item.tax}
                                                            onChange={(e) => updateItem(item.id, 'tax', e.target.value)} />
                                                    </td>
                                                    <td>
                                                        <input type="number" className="Invoice-discount-input" value={item.discount}
                                                            onChange={(e) => updateItem(item.id, 'discount', e.target.value)} />
                                                    </td>
                                                    <td>
                                                        <input type="text" className="Invoice-amount-input Invoice-disabled" value={formatCurrency(item.total || 0)} Invoice-disabled />
                                                    </td>
                                                    <td className="text-center">
                                                        <button className="Invoice-btn-delete-row" onClick={() => removeItem(item.id)}>
                                                            <Trash2 size={16} />
                                                        </button>
                                                    </td>
                                                </tr>
                            ...
                            <div className="Invoice-totals-layout">
                                <div className="Invoice-totals-spacer"></div>
                                <div className="Invoice-totals-box">
                                    <div className="Invoice-t-row">
                                        <span>Sub Total:</span>
                                        <span>{formatCurrency(totals.subTotal)}</span>
                                    </div>
                                    <div className="Invoice-t-row">
                                        <span>Discount:</span>
                                        <span className="Invoice-text-red-500">-{formatCurrency(totals.discount)}</span>
                                    </div>
                                    <div className="Invoice-t-row">
                                        <span>Tax Total:</span>
                                        <span>{formatCurrency(totals.tax)}</span>
                                    </div>
                                    <div className="Invoice-t-row Invoice-total">
                            ...
                            <div className="Invoice-form-footer-grid">
                                <div className="Invoice-bank-terms-col">
                                    <label className="Invoice-section-label">Bank Details</label>
                                    <div className="Invoice-bank-details-box">
                                        <input type="text" className="Invoice-bank-input" placeholder="Bank Name" value={bankDetails.bankName} onChange={(e) => setBankDetails({ ...bankDetails, bankName: e.target.value })} />
                                        <input type="text" className="Invoice-bank-input" placeholder="Account No" value={bankDetails.accNo} onChange={(e) => setBankDetails({ ...bankDetails, accNo: e.target.value })} />
                                        <input type="text" className="Invoice-bank-input" placeholder="Account Holder" value={bankDetails.holderName} onChange={(e) => setBankDetails({ ...bankDetails, holderName: e.target.value })} />
                                        <input type="text" className="Invoice-bank-input" placeholder="IFSC / Swift" value={bankDetails.ifsc} onChange={(e) => setBankDetails({ ...bankDetails, ifsc: e.target.value })} />
                                    </div>
                                    <div className="mt-4">
                                        <label className="Invoice-section-label">Attachments</label>
                                        <div className="Invoice-attachments-row">
                                            <button className="Invoice-btn-upload-small">
                                                <span className="Invoice-icon">📷</span> Photos
                                            </button>
                                            <button className="Invoice-btn-upload-small">
                                                <span className="Invoice-icon">📎</span> Files
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                <div className="Invoice-notes-col">
                                    <label className="Invoice-section-label">Notes</label>
                                    <textarea className="Invoice-notes-area" value={notes} onChange={(e) => setNotes(e.target.value)}></textarea>
                                </div>
                            </div>
                            <div className="Invoice-terms-section mt-4">
                                <label className="Invoice-section-label">Terms & Conditions</label>
                                <textarea className="Invoice-terms-area" value={terms} onChange={(e) => setTerms(e.target.value)} />
                            </div>
                        </div>
                            ...
                            <div className="Invoice-source-list-container">
                                ...
                                <div className="Invoice-source-items-list">
                                    {activeOrders.map(order => (
                                        <div key={order.id} className="Invoice-source-item-row" onClick={() => { handleSelectOrder(order); setShowAddModal(true); }}>
                                ...
                                <button className="Invoice-btn-back-sel" onClick={() => setCreationMode('direct')}>Back</button>
                            </div>
                        )}
                            ...
                            <div className="Invoice-source-list-container">
                                ...
                                <div className="Invoice-source-items-list">
                                    {activeChallans.map(dc => (
                                        <div key={dc.id} className="Invoice-source-item-row" onClick={() => { handleSelectChallan(dc); setShowAddModal(true); }}>
                                            <div className="Invoice-source-info">
