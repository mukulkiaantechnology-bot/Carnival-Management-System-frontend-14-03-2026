...
const PurchaseReport = () => {
    ...
    const summaryCards = [
        { id: 1, label: 'Total Purchase', value: formatCurrency(summaryStats.totalAmount), color: 'blue', icon: ShoppingBag },
        { id: 2, label: 'Paid Amount', value: formatCurrency(summaryStats.totalPaid), color: 'green', icon: CheckCircle2 },
        { id: 3, label: 'Unpaid Amount', value: formatCurrency(summaryStats.totalUnpaid), color: 'orange', icon: AlertCircle },
        { id: 4, label: 'Overdue', value: formatCurrency(summaryStats.overdue), color: 'red', icon: XCircle },
    ];
                    ...
                    {loading ? (
                        ...
                        <table className="report-table">
                            ...
                            <tbody>
                                {reportData.map((row) => (
                                    <tr key={row.id}>
                                        <td className="font-mono App-text-primary text-sm cursor-pointer hover:underline" onClick={() => navigate('/company/purchases/bill', { state: { targetBillId: row.billId } })}>
                                            {row.billNumber}
                                        </td>
                                        ...
                                        <td className="text-right text-gray-600">{formatCurrency(row.rate)}</td>
