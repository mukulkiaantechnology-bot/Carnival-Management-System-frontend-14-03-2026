                            ...
                            {loading ? (
                                ...
                                quotations.map(q => (
                                    <tr key={q.id}>
                                        ...
                                        <td>{formatCurrency(q.totalAmount || 0)}</td>
                    ...
                    <div className="PurchaseQuotation-form-modal">
                        <div className="PurchaseQuotation-header-simple">
                            ...
                            <div className="flex items-center gap-3">
                                {isViewMode && (
                                    <button className="PurchaseQuotation-btn-print" onClick={() => window.print()}>
                        ...
                        <div className="PurchaseQuotation-body-scrollable">
                            <div className="PurchaseQuotation-section-grid">
                                <div className="PurchaseQuotation-company-section">
                                    <div className="PurchaseQuotation-logo-upload-box">
                                        {companyDetails.logo ? (
                                            <img
                                                src={companyDetails.logo.startsWith('http') ? companyDetails.logo : `${BASE_URL}/${companyDetails.logo.replace(/\\/g, '/')}`}
                                                alt="Logo"
                                                style={{ width: '100%', height: '100%', objectFit: 'contain' }}
                                            />
                                        ) : (
                                            <h1 className="PurchaseQuotation-logo-text">LOGO</h1>
                                        )}
                                    </div>
                                    <input type="text" className="PurchaseQuotation-input" placeholder="Company Name" disabled={isViewMode}
                                        value={companyDetails.name} onChange={e => setCompanyDetails({ ...companyDetails, name: e.target.value })} />
                                    <input type="text" className="PurchaseQuotation-input" placeholder="Company Address" disabled={isViewMode}
                                        value={companyDetails.address} onChange={e => setCompanyDetails({ ...companyDetails, address: e.target.value })} />
                                    <input type="text" className="PurchaseQuotation-input" placeholder="Company Email" disabled={isViewMode}
                                        value={companyDetails.email} onChange={e => setCompanyDetails({ ...companyDetails, email: e.target.value })} />
                                </div>
                                <div className="PurchaseQuotation-meta-section">
                                    <div className="PurchaseQuotation-meta-row">
                                    ...
                                    <div className="PurchaseQuotation-quotation-badge">QUOTATION</div>
                                </div>
                            </div>

                            <div className="PurchaseQuotation-divider" />

                            <div className="PurchaseQuotation-vendor-section">
                                <label className="form-label-sm mb-2 block font-semibold text-gray-700">Quotation To</label>
                                <select className="PurchaseQuotation-select-large mb-3" value={vendorId} disabled={isViewMode} onChange={(e) => {
                                ...
                                <div className="PurchaseQuotation-vendor-details-row">
                                    <input type="text" disabled className="PurchaseQuotation-input" placeholder="Billing Address" value={vendors.find(v => v.id === parseInt(vendorId))?.billingAddress || ''} />
                                    <input type="text" disabled className="PurchaseQuotation-input" placeholder="Email Address" value={vendors.find(v => v.id === parseInt(vendorId))?.email || ''} />
                                    <input type="text" disabled className="PurchaseQuotation-input" placeholder="Phone Number" value={vendors.find(v => v.id === parseInt(vendorId))?.phone || ''} />
                                </div>
                            </div>

                            <div className="PurchaseQuotation-items-section">
                                {!isViewMode && (
                                ...
                                <div className="table-responsive">
                                    <table className="PurchaseQuotation-items-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            {items.map(item => (
                                                <tr key={item.id}>
                                                    <td>
                                                        <select
                                                            className="PurchaseQuotation-select-sm"
                                                    ...
                                                    <td>
                                                        <input type="text" className="PurchaseQuotation-amount-input disabled" value={formatCurrency(item.total)} disabled />
                                                    </td>
                            ...
                            <div className="PurchaseQuotation-footer-grid">
                                <div className="PurchaseQuotation-bank-details-box">
                                    ...
                                    {vendorId ? (
                                        <>
                                            <div className="PurchaseQuotation-bank-row"><strong>Bank:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankNameBranch || 'N/A'}</div>
                                            <div className="PurchaseQuotation-bank-row"><strong>Account No:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankAccountNumber || 'N/A'}</div>
                                            <div className="PurchaseQuotation-bank-row"><strong>IFSC:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankIFSC || 'N/A'}</div>
                                            <div className="PurchaseQuotation-bank-row"><strong>Holder:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.accountName || 'N/A'}</div>
                                        </>
                                    ) : (
                                ...
                                <div className="PurchaseQuotation-totals-box">
                                    ...
                                    <div className="PurchaseQuotation-t-row total">
                            ...
                            <div className="PurchaseQuotation-notes-terms-container">
                                <div className="PurchaseQuotation-notes-box">
                                    <label className="section-label mb-2 block text-sm font-semibold">Notes</label>
                                    <textarea className="PurchaseQuotation-textarea" placeholder="Enter notes..." value={notes} onChange={(e) => setNotes(e.target.value)} disabled={isViewMode}></textarea>
                                </div>
                                <div className="PurchaseQuotation-terms-box">
                                    <label className="section-label mb-2 block text-sm font-semibold">Terms & Conditions</label>
                                    <textarea className="PurchaseQuotation-textarea" placeholder="Enter terms..." value={terms} onChange={(e) => setTerms(e.target.value)} disabled={isViewMode}></textarea>
                                </div>
                            </div>

                            {!isViewMode && (
                                <div className="PurchaseQuotation-attachments-section">
                                    ...
                                    />
                                    <button className="PurchaseQuotation-attach-btn" onClick={() => fileInputRef.current.click()}>
                        ...
                        <div className="PurchaseQuotation-footer-simple">
                            {editingId && !isViewMode && quotations.find(q => q.id === editingId)?.status !== 'ACCEPTED' && (
                                <button className="PurchaseQuotation-btn-plain text-blue-600 border-blue-200 mr-auto hover:bg-blue-50"
                            ...
                            {!isViewMode && (
                                <button className="PurchaseQuotation-btn-primary" onClick={handleSave}>
