                    ...
                    <div className="PurchaseOrder-form-modal">
                        <div className="PurchaseOrder-header-simple">
                            ...
                            <div className="flex items-center gap-3">
                                {isViewMode && (
                                    <button className="PurchaseOrder-btn-print" onClick={() => window.print()}>
                        ...
                        <div className="PurchaseOrder-body-scrollable">
                            ...
                            {!editingId && (
                                <div className="purchase-toggle-wrapper">
                                    ...
                                    {orderType === 'quotation' && (
                                        <div style={{ width: '100%' }}>
                                            <select
                                                className="PurchaseOrder-select-large"
                                                style={{ width: '100%' }}
                                            ...
                                            >
                                                ...
                                                {quotations.filter(q => {
                                                    ...
                                                    <option key={q.id} value={q.id}>
                                                        {q.quotationNumber} - {q.vendor?.name} - {formatCurrency(q.totalAmount)}
                                                    </option>
                                                ))}
                                            </select>
                                        </div>
                            ...
                            <div className="PurchaseOrder-section-grid">
                                <div className="PurchaseOrder-company-section">
                                    <div className="PurchaseOrder-logo-upload-box">
                                        {companyDetails.logo ? (
                                            <img
                                            ...
                                            <h1 className="PurchaseOrder-logo-text">LOGO</h1>
                                        )}
                                    </div>
                                    <input type="text" className="PurchaseOrder-input" placeholder="Company Name" disabled={isViewMode}
                                        value={companyDetails.name} onChange={e => setCompanyDetails({ ...companyDetails, name: e.target.value })} />
                                    <input type="text" className="PurchaseOrder-input" placeholder="Company Address" disabled={isViewMode}
                                        value={companyDetails.address} onChange={e => setCompanyDetails({ ...companyDetails, address: e.target.value })} />
                                    <input type="text" className="PurchaseOrder-input" placeholder="Company Email" disabled={isViewMode}
                                        value={companyDetails.email} onChange={e => setCompanyDetails({ ...companyDetails, email: e.target.value })} />
                                </div>
                                <div className="PurchaseOrder-meta-section">
                                    <div className="PurchaseOrder-meta-row">
                                        <label>PO No.</label>
                            ...
                            <hr className="PurchaseOrder-divider" />

                            <div className="PurchaseOrder-vendor-section">
                                <div className="form-group mb-2">
                                    <label className="form-label-sm">Select Vendor</label>
                                    <select className="PurchaseOrder-select-large" value={vendorId} onChange={(e) => setVendorId(e.target.value)} disabled={!!sourceData || isViewMode}>
                            ...
                            <div className="PurchaseOrder-items-section">
                                {!isViewMode && (
                                ...
                                <div className="table-responsive">
                                    <table className="PurchaseOrder-items-table">
                                        <thead>
                                            <tr>
                                        ...
                                        <tbody>
                                            {items.map(item => (
                                                <tr key={item.id}>
                                                    <td>
                                                        <select
                                                            className="PurchaseOrder-select-sm"
                                                    ...
                                                    <td>
                                                        <input type="text" className="PurchaseOrder-amount-input disabled" value={formatCurrency(item.total)} disabled />
                                                    </td>
                            ...
                            <div className="PurchaseOrder-footer-grid">
                                <div className="PurchaseOrder-bank-details-box">
                                    ...
                                    {vendorId ? (
                                        <>
                                            <div className="PurchaseOrder-bank-row"><strong>Bank:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankNameBranch || 'N/A'}</div>
                                            <div className="PurchaseOrder-bank-row"><strong>Account No:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankAccountNumber || 'N/A'}</div>
                                            <div className="PurchaseOrder-bank-row"><strong>IFSC:</strong> {vendors.find(v => v.id === parseInt(vendorId))?.bankIFSC || 'N/A'}</div>
                                        </>
                                    ) : (
                                ...
                                <div className="PurchaseOrder-totals-box">
                                    ...
                                    <div className="PurchaseOrder-t-row total">
                            ...
                            <div className="PurchaseOrder-notes-terms-container">
                                <div className="PurchaseOrder-notes-box">
                                    <label className="section-label mb-2 block text-sm font-semibold">Notes</label>
                                    <textarea className="PurchaseOrder-textarea" value={notes} onChange={(e) => setNotes(e.target.value)} disabled={isViewMode}></textarea>
                                </div>
                                <div className="PurchaseOrder-terms-box">
                                    <label className="section-label mb-2 block text-sm font-semibold">Terms & Conditions</label>
                                    <textarea className="PurchaseOrder-textarea" placeholder="Enter terms..." value={terms} onChange={(e) => setTerms(e.target.value)} disabled={isViewMode}></textarea>
                                </div>
                            </div>
                        ...
                        <div className="PurchaseOrder-footer-simple">
                            {editingId && !isViewMode && (
                                <>
                                    <button className="PurchaseOrder-btn-plain text-blue-600 border-blue-200 mr-2 hover:bg-blue-50"
                                        onClick={() => handleCreateGRN(orders.find(o => o.id === editingId))}>
                                        Convert to GRN
                                    </button>
                                    <button className="PurchaseOrder-btn-plain text-blue-600 border-blue-200 mr-auto hover:bg-blue-50"
                            ...
                            {!isViewMode && (
                                <button className="PurchaseOrder-btn-primary" onClick={handleSave}>
                ...
                <div className="PurchaseOrder-modal-overlay">
                    <div className="delete-confirmation-box" style={{ background: 'white', padding: '24px', borderRadius: '8px', width: '400px' }}>
                        ...
                        <div className="delete-modal-footer" style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
                            <button className="PurchaseOrder-btn-plain" onClick={() => setShowDeleteConfirm(false)}>Cancel</button>
                            <button className="PurchaseOrder-btn-delete-row" style={{ width: 'auto', padding: '8px 16px', borderRadius: '4px', background: '#ef4444', color: 'white' }} onClick={confirmDelete}>Delete</button>
                        </div>
                    </div>
                </div>
            )}
